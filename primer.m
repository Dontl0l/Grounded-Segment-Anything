clc;
clear variables;
%входные данные.     Если узлов нет, но решаем другим методом
R1=200;     %задача из 3 семинара. Ответ сошелся (у меня другое направление токов, поэтому знак другой)
E1=600;           %возожно другой знак
E3=-100i;
C2=100i;
L2=-200i;
C4=100i;
...
J5=-2-2i;
L6=-100i;
E6=100i;                     %почему-то при транспанировании получается другой знак в коэф при i
...
% то эти элементы записываются на одной строке
b1=[0 0 0 E3 0]';
c1=[R5 0 0 0 J5]';
d1=[0 C2 L2 0 0]';
e1=[0 C4 0 0 0]';
f1=[0 0 L6 E6 0]';
m_element=[a1 b1 c1 d1 e1 f1]; %это матрица элементов на ветвях
%конец входных данных
vetvi=size(graf, 2);     %кол-во ветвей
I=zeros(1, vetvi);         %массив токов на ветвях (ответ)
...
for yy=1:vetvi
        ...
        if kolv_J1==1
        kolv_J=kolv_J+1;                   %кол-во ветвей, на которых есть источник тока. Понадобится для того, чтобы понять, сколько уравнений нам нужно(не в этом коде)
        end

%ищу ветви, на которых есть J
for tt=1:nn
if m_element(5, yy, tt)~=0    %если на ветви есть источник тока
    I(yy)=sum_J (yy);          %ток на ветви равен сумме токов на ист. тока
    if xv==0
    dop_u_kolv=dop_u_kolv+1;    %коли-во дополнительных (почти нулевых) уравнений ( кол-во ветвей, на которых есть ист. тока
    xv=1;
    end
end
end
xv=0;
end           %капец, работает, я в шоке
